...
```{r}
all_data_vcf <- vcfR::read.vcfR("../jSLE/data/riskfactor.vcf", convertNA = T)

vcf_new <- readVcf("../jSLE/data/riskfactor.vcf")
...
combined_info_allele <- left_join(all_data_info_split, geno_vcf_mat.na, join_by("SNP" == "SNP"))
...
for(i in colnames(combined_info_allele_na2[5:333])){
  ...
  jsle_data_final[, i] <- mapply(FUN = "*",
                                 combined_info_allele_na3[[i]],
                                 combined_info_allele_na3[["ANN_value"]])
...
demos <- read.csv("../../jSLE/data/JSLE_combined_meta_treatment_updated24_.csv")
...
p1
#ggsave(here("figures/gene_level_PCA_not_corrected_all_eth.png"), p1, width = 9, height = 7)

#empty_df_na$ID <- rownames(empty_df_na)
...
demos$Patient_ID <- as.character(demos$Patient_ID)
demos <- demos %>% filter(Tag == "include")
