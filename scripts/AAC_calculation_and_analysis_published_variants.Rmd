...
# just published risk snps
```{r}

pub_vars <- read.csv(here("data/variant_list.csv"))

combine_info_pub <- combine_info %>% filter(SNP %in% pub_vars$published_snps)
...
for(i in colnames(combine_info_pub[1:329])){
print(i)
multiply_matrix[, i] <- mapply(FUN = "*",
combine_info_pub[[i]],
combine_info_pub[["ANN_value"]])
}
...
#minors_weighted <- multiply_matrix_t[,16:17]
...
multiply_matrix <- combine_info_pub
...
#minors_weighted <- multiply_matrix_t[,4101:4102]
minors_weighted_pub <- multiply_matrix_t[,16:17]
View(minors_weighted_pub)
```
...
aac_patients <- AAC_freq_filter.na$Patient_ID

AAC_freq_filter.na_pub <- left_join(AAC_freq_filter.na, minors_weighted_pub, by= "Patient_ID")
...
#leaves 21 variables
vars_to_test <- vars_to_test[! vars_to_test %in% c("Gender", "Ethnicity_def", "FamilyHistoryAutoimmuneDiseases", "LSLICCTotal", "SLEDAI_first", "SLEDAI_highest", "BILAG_highest", "BILAG_first","Flare_count_AnySEVERity", "Flare_per_year", "SLICC_last", "FU_more_1y")]
...
for (i in vars_to_test){ 
  model <- glm(paste("sum.y ~", i, paste("+ Gender + FamilyHistoryAutoimmuneDiseases")), family = "poisson", data=AAC_freq_filter.na_pub)
...
model <- glm(sum.y ~ Treament_Group*Ethnicity_def + Gender + FamilyHistoryAutoimmuneDiseases, family = "poisson", data=AAC_freq_filter.na_pub)
anova <- anova(model, test="Chi")
...
correlation <- cor.test(AAC_freq_filter.na_pub$sum.y, AAC_freq_filter.na_pub$Diagnosis_Age, method = "pearson")
correlation <- cor.test(AAC_freq_filter.na_pub$sum.y, AAC_freq_filter.na_pub$BILAG_highest, method = "pearson")
correlation <- cor.test(AAC_freq_filter.na_pub$sum.y, as.numeric(AAC_freq_filter.na_pub$SLEDAI_highest), method = "pearson")
correlation <- cor.test(AAC_freq_filter.na_pub$sum.y, AAC_freq_filter.na_pub$SLICC_last, method = "pearson")

#adust pvalues using fdr
adjust <- p.adjust(0.241, n=4, method = "fdr")
...
ethnicities <- unique(AAC_freq_filter.na_pub$Ethnicity_def)
...
for (ethnicity in ethnicities) {
  ...
  subset_df <- subset(AAC_freq_filter.na_pub, Ethnicity_def == ethnicity)

  # Calculate correlation between count and diagnosis age within the current ethnicity
  correlation <- cor.test(subset_df$sum.y, subset_df$Diagnosis_Age, method = "pearson")
  
  # Store the correlation result
  correlation_results[[ethnicity]] <- correlation
}

cor.test(AAC_freq_filter.na_pub$Diagnosis_Age, AAC_freq_filter.na_pub$sum.)

correlation_results

#caucasian
p.adjust(0.5697, n=5, method="fdr")
#south asian
p.adjust(0.7065, n=5, method="fdr")
#any other asian
p.adjust(0.7532, n=5, method="fdr")
#black aff/carr
p.adjust(0.319, n=5, method="fdr")
#east asian
p.adjust(0.1913, n=5, method="fdr")
